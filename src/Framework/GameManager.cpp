#include "runaway/Framework/GameManager.hpp"
...
#include "runaway/GameWorld/GameWorld.hpp"  // To access GameWorld's GetScore()
...
// Helper function to display prompt text
...
void GameManager::Play(int argc, char **argv, std::shared_ptr<WorldBase> world)
{
	...
	glutMotionFunc([](int x, int y) {
		GameManager::Instance().MouseMoveEvent(x, WINDOW_HEIGHT - y);
	});
	...
	// Initialize SpriteManager. Sprites will be loaded during the starting screen.
...
void GameManager::Update()
{
	...
	case GameManager::GameState::TITLE:
		if (!SpriteManager::Instance().IsLoaded())
		{
			...
			{
				Prompt("ERROR: Failed to load assets!",
					("You may need to change ASSET_DIR in utils.hpp. It's currently " + ASSET_DIR).c_str());
				break;
			}
		}
...
void GameManager::MouseMoveEvent(int x, int y)
{
...
std::size_t GameManager::DrawOneObject(ImageID imageID, AnimID animID, double x, double y, std::size_t frame) const
{
	...
	int spriteRow = frame / spriteInfo.cols;
	int spriteCol = frame % spriteInfo.cols;
...
// --------------- Key Point: Display final Score here -----------
void GameManager::ShowGameOver() const
{
	...
	// Get the final score from m_world
	// We know m_world is a std::shared_ptr<WorldBase>,
	// but the actual type is GameWorld, so we can use dynamic_pointer_cast
	auto gw = std::dynamic_pointer_cast<GameWorld>(m_world);
	int finalScore = 0;
	if (gw)
	{
		finalScore = gw->GetScore();
	}
	
	// First, display all TextBase objects created in the game, so if GameWorld also has some texts like "GameOver", they can be seen
	TextBase::DisplayAllTexts(
		[ = ](int x, int y, const std::string &text, double r, double g, double b, bool centering) {
			glPushAttrib(GL_CURRENT_BIT);
			glColor3f(r, g, b);
	...
	// Then display a prompt with the score in the center of the screen
	glPushAttrib(GL_CURRENT_BIT);
	glColor3f(1.0f, 1.0f, 1.0f);
	
	// Concatenate text
	std::string finalScoreText = "Your score: " + std::to_string(finalScore) + "  Press Enter to restart.";
	displayText(
		NormalizeCoord(WINDOW_WIDTH / 2, WINDOW_WIDTH),
